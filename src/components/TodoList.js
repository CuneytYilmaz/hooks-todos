            ...
            <ul className='list-reset text-white p-0'>
                {state.todos.map(todo => (
                    <li key={todo.id}
                        ...
                        <span
                            onDoubleClick={async () => {
                                const response = await axios.patch(`https://hooks-api.cuneytyilmaz.now.sh/todos/${todo.id}`,
                        ...
                        <button onClick={async () => {
                            await axios.delete(`https://hooks-api.cuneytyilmaz.now.sh/todos/${todo.id}`)
